...
def connect_database():
    try:
        connection = pymysql.connect(host='localhost', user='root', password='') #Please use the password you used when you install MYSQL
...
def select_data(event, empid_entry, name_entry, email_entry,
                ...
                education_combobox,
                work_shift_combobox,
                address_text, doj_date_entry,
                salary_entry, user_type_combobox,
                password_entry):
...
def add_employee(empid, name, email, gender, dob, contact, employment_type, education, work_shift, address, doj, salary,
                 user_type, password):
    if (
            empid == '' or name == '' or email == '' or gender == 'Select Gender' or dob == '' or contact == '' or employment_type == 'Select type' or
...
def clear_fields(empid_entry, name_entry, email_entry,
                 ...
                 address_text, doj_date_entry, salary_entry, user_type_combobox, password_entry, check):
        ...
        try:
            ...
            cursor.execute(
                ...
                (name, email, gender, dob, contact, employment_type, education, work_shift, address, doj,
                 salary, user_type, password, empid))
...
def employee_form(window):
    ...
    employee_frame = Frame(window, width=1250, height=700, bg='white')
    employee_frame.place(x=275, y=100)
    heading_label = Label(employee_frame, text='Manage Employees Details', font=('times new roman', 16, 'bold'),
                          bg='#0f4d7d', fg='white')
    ...
    search_button = Button(search_frame, text='Search', font=('times new roman', 11), width=10, cursor='hand2',
                           fg='white', bg='#0f4d7d',
                           command=lambda: search_employee(search_combobox.get(), search_entry.get()))
    search_button.grid(row=0, column=2, padx=20)
    show_button = Button(search_frame, text='Show All', font=('times new roman', 11), width=10, cursor='hand2',
    ...
    employee_treeview = ttk.Treeview(top_frame, columns=(
        ...
        'doj', 'salary', 'user_type'), show='headings',
    ...
    button_frame.place(x=250, y=580)

    add_button = Button(button_frame, text='Add', font=('times new roman', 11), width=10, cursor='hand2',
                        fg='white', bg='#0f4d7d',
                        command=lambda: add_employee(empid_entry.get(), name_entry.get(), email_entry.get(),
                                                     ...
                                                     salary_entry.get(), user_type_combobox.get(),
                                                     password_entry.get()))

    add_button.grid(row=0, column=0, padx=20)

    update_button = Button(button_frame, text='Update', font=('times new roman', 11), width=10, cursor='hand2',
                           fg='white', bg='#0f4d7d',
    ...
    delete_button = Button(button_frame, text='Delete', font=('times new roman', 11), width=10, cursor='hand2',
                           fg='white', bg='#0f4d7d',
                           command=lambda: delete_employee(empid_entry.get(), ))
    delete_button.grid(row=0, column=2, padx=20)

    clear_button = Button(button_frame, text='Clear', font=('times new roman', 11), width=10, cursor='hand2',
