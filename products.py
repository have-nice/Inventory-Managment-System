...
from employees import connect_database, clear_fields, treeview_data
...
def product_form(window):
    global back_image
    product_frame = Frame(window, width=1250, height=700, bg='white')
    product_frame.place(x=275, y=100)
    ...
    supplier_combobox.grid(row=2, column=1,pady=25)
    ...
    status_combobox.grid(row=6, column=1, pady=25)
    status_combobox.set('Select Status')
    ...
    treeview_frame.place(x=480, y=125, width=600,height=480)
    ...
    treeview.heading('supplier', text='Product Supplier')
    ...
    treeview_data(treeview)
    treeview.bind('<ButtonfRelease-1>',lambda event:select_data (event,treeview,category_combobox,supplier_combobox,name_entry,price_entry,quantity_entry,status_combobox))
